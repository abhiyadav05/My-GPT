...
function Sidebar() {
  ...
  return (
    <div className='flex flex-col h-screen min-w-72 p-4 dark:bg-gradient-to-b
     from-[#242124]/30 to-[#000000]/30 border-r border-[#80609f]/30 backdrop-blur-3xl transition-all duration-500
      max-md:absolute left-0 z-1'>
     {/* Logo */}
    <div className='flex justify-center items-center gap-4'>
       <img src={assets.technology_logo} 
     alt='' className='w-full  max-w-12'/>
     <span className='text-2xl text-shadow-black'>My-GPT</span>
     ...
     <div className='flex items-center gap-2 p-3 mt-3
      border border-gray-400 dark:border-white/20 rounded-md'>
      <img src={assets.search_icon}  className='w-4 not-dark:invert' alt="" />
      <input onChange={(e)=>setSearch(e.target.value)} value={search}
      type="text" placeholder='Search Conversation' className='text-xs placeholder:text-gray-400 outline-none' />
     ...
     <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
      ...
      : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat)=>(
        <div key={chat.id} className='p-2 px-4
     ...
     {/* Credit Section */}
     <div onClick={()=>{navigate('/payment')}} className='flex items-center gap-2 p-3 mt-4 border border-gray-300
      dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
      <img src={assets.diamond_icon} alt="" className='w-4.5 dark:invert'/>
      <div className='flex flex-col text-sm'>
        <p>Credits : {user?.credits}</p>
        <p className='text-xs text-gray-400'>Purchase credits to use My-GPT</p>
      </div>
     </div>
